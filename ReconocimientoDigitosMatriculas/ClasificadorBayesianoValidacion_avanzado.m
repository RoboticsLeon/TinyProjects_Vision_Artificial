        ...
        if( ((aspectRatioLicensePlate*0.9)<aspectRatio) && (aspectRatio<(aspectRatioLicensePlate*1.1)) )
           ...
           if(caractRectAzul.Centroid(1)>CentroidMat.Centroid(1))
            ...
            for digitos=1:length(centroidInfo)
                ...
                if(flagInv)
                    text(centroidInfo(digitos).Centroid(1),centroidInfo(digitos).Centroid(2),characterClaseRes,"Color",[1 0 0],"FontSize",44,"HorizontalAlignment","center","VerticalAlignment","middle")
                else
