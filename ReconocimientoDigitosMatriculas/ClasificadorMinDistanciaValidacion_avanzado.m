...
for imagen = 1:Nimagenes %Utilizar cada una de las imágenes de validación
    ...
    str_path = strcat('imagenes\validacion',str_imagen,'.jpg');
    ...
    for i=1:max(max(binPlantillaEtiq)) %Analizo cada uno de los objetos de la plantilla
        ...
        if( ((aspectRatioLicensePlate*0.9)<aspectRatio) && (aspectRatio<(aspectRatioLicensePlate*1.1)) )
            ...
            for j=1:max(max(binPlantilla_Digits_Etiq)) %Analizo cada uno de los dígitos de la matrícula
                ...
                xVal(1,1) = max(OccupiedArea.Area)/(yNorm*xNorm); %ratio de ocupación (Caracteristica 1) %Uso max() para evitar posibles areas pequeñas sueltas
                ...
                if(nivel>=1) %C2-C5
                    ...
                    for k=1:4
                        ...
                        xVal(k+1,1) = max(OccupiedArea.Area)/(tamX*tamY); %ratio de ocupación (Caracteristicas 2-5)
                    end
                end
                ...
                if(nivel>=2) %C6-C13
                    ...
                    for k=1:8
                        ...
                        xVal(k+5,1) = max(OccupiedArea.Area)/(tamX*tamY); %ratio de ocupación (Caracteristicas 6-13)
                    end
                end

                MatrizPatronesVal(:,muestra) = xVal; %almaceno patron de caracteristicas
            end
            ...
            for digitos=1:length(centroidInfo)
                ...
                if(flagInv)
                    text(centroidInfo(digitos).Centroid(1),centroidInfo(digitos).Centroid(2),characterClaseRes,"Color",[1 0 0],"FontSize",20,"HorizontalAlignment","center","VerticalAlignment","middle")
                else
                    text(centroidInfo(digitos).Centroid(1),centroidInfo(digitos).Centroid(2),characterClaseRes,"Color",[0 1 0],"FontSize",20,"HorizontalAlignment","center","VerticalAlignment","middle")
                end
            end
            hold off
        end
